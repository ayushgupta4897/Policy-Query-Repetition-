...
We evaluate instruction adherence across two levels of policy complexity (Table~\ref{tab:policies}).
...
To comprehensively evaluate adherence, we define 17 specific violation checks grouped into 6 categories, outlined in Table~\ref{tab:categories}.
...
Our evaluation spans a diverse set of 7 modern LLMs, including proprietary and open-weights models with varying architectures (Table~\ref{tab:models}).
...
As a foundational baseline, we first evaluate the overarching efficacy of PQR against standard prompting. As shown in Table~\ref{tab:simple_results} and Figure~\ref{fig:simple_results}, PQR yields substantial improvements across almost all evaluated autoregressive models on simple policies, achieving a 52.7\% overall reduction in formatting violations. Notably, Llama-3.3-70B achieves perfect compliance (0.0\% violation rate).

This trend largely holds for complex, multi-section policies (Table~\ref{tab:complex_results}), where PQR drives a 45.5\% overall improvement. Frontier models like GPT-4o and GPT-4.1 demonstrate exceptional responsiveness to the intervention, whereas Gemini-2.0-Flash exhibits resistance to positional manipulation, remaining essentially unchanged across both complexity levels.
...
The PQR prompt is tokenized into 263 tokens, divided into four semantic regions (detailed in Table~\ref{tab:attention} and visualized in Figure~\ref{fig:attention_routing}):

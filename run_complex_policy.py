#!/usr/bin/env python3
"""
Runner for complex policy experiments.
Tests if PQR helps more when policies are longer and more complex.
"""
...
from complex_policy_tests import ALL_COMPLEX_TESTS
...
def build_pqr_prompt(policy: str, query: str) -> list[dict]:
    """PQR prompt - for complex policies, we include key reminders."""
    repeated_user_content = f"""{query}

═══════════════════════════════════════════════════════════════════
CRITICAL REMINDER - BEFORE YOU RESPOND, RE-READ THESE KEY RULES:

{policy}
═══════════════════════════════════════════════════════════════════

Now answer the original query while following ALL rules above strictly."""
...
def run_single_test(tc, mode: str, sample: int, model_name: str) -> dict:
    ...
    return {
        ...
        "reason": reason,
        "response": response[:400] if response else ""
    }


def run_complex_experiment(models: list[str] = None, samples_per_case: int = 5):
    if models is None:
        models = ACTIVE_MODELS
    
    test_cases = ALL_COMPLEX_TESTS
    modes = ["baseline", "pqr"]
    ...
    print("=" * 80)
    print("COMPLEX POLICY PQR EXPERIMENT")
    print("=" * 80)
    print(f"Policy: MEGA_POLICY (~8000 chars, 7 sections)")
    ...
    for model in models:
        ...
        # Per-category
        categories = set(r["category"] for r in mdata)
        cat_results = {}
        for cat in categories:
            cat_b = [r for r in baseline if r["category"] == cat]
            cat_p = [r for r in pqr if r["category"] == cat]
            cb = sum(1 for r in cat_b if r["is_violation"])
            cp = sum(1 for r in cat_p if r["is_violation"])
            cat_imp = ((cb - cp) / cb * 100) if cb > 0 else 0
            cat_results[cat] = {"baseline": cb, "pqr": cp, "improvement": cat_imp}
        ...
        for cat, stats in cat_results.items():
            print(f"      • {cat}: {stats['baseline']} → {stats['pqr']} ({stats['improvement']:.0f}%)")
        print()
    ...
    print("-" * 50)
    print(f"OVERALL: {total_b}/{total_bt} ({total_b/total_bt*100:.1f}%) → {total_p}/{total_pt} ({total_p/total_pt*100:.1f}%)")
    ...
    results_file = os.path.join(RESULTS_DIR, f"complex_policy_{timestamp}.json")
    with open(results_file, "w") as f:
        json.dump({
            "experiment": "complex_policy",
            "results": all_results,
            "summary": summary,
    ...
    return all_results, summary, overall_imp
...
if __name__ == "__main__":
    ...
    run_complex_experiment(models=args.models, samples_per_case=args.samples)
